...
- [Comunes](#historias-de-usuario-comunes)
- [Administradores)](#historias-de-usuario-administradores)
- [Médicos](#historias-de-usuario-médicos)
- [Pacientes](#historias-de-usuario-pacientes)
- [Farmacias](#historias-de-usuario-farmacias)

# HISTORIAS DE USUARIO (Comunes)

## HU-C1. Login de usuarios
...
### Criterios de aceptación
* Se mostrará una lista con las _citas_ registradas para el día de hoy, indicando sus datos esenciales (nombre de _paciente_, fecha y hora, duración, estado). 
* Seleccionando una de las _citas_ con estado `PLANIFICADA` se accede al formulario de "Atención de _cita_" (ver historia HU-M3).
* Se mostrará un botón "Ausente" para cada _cita_ con estado `PLANIFICADA`, mediante el cual se marcará esa cita como `AUSENTE` en caso de que no se presente el _paciente_, actualizando el listado de _citas_ de la agenda del _médico_.
...
* Se mostrará la lista de _prescripciones_ en vigor para el _paciente_ actual (su "fecha de fin" es igual o posterior a la fecha actual). Para cada _prescripción_ se mostrarán los datos del _medicamento_ prescrito, la dosis establecida y las fechas de inicio y fin de la _prescripción_. La lista estará ordenada por fecha de inicio de la _prescripción_.
* Para cada _prescripción_ de ese listado se mostrará un botón "Anular" con el que se elimina (1) la correspondiente _prescripción_ (marcándola en estado `INACTIVA`) y (2) la lista de _recetas_ previstas para la misma (marcando cada una de ellas como `ANULADA`). Se actualizará la lista de _prescripciones_ tras estos cambios.
* Se podrá acceder a la opción de confeccionar una nueva _prescripción_ para el _paciente_ concreto atendido en la _cita_ actual pasando al formulario "Creación de prescripciones" (historia HU-M4)
* Se mostrará un botón "Cita completada" mediante el cual, una vez finalizada la _cita_ actual, esta se marca con el estado `COMPLETADA`.
...
### Descripción
El **sistema**, como resultado de la confección de una nueva _prescripción_ por parte de un _médico_, creará de forma automática un "plan de recetas" para el correspondiente _paciente_, que consistirá en una serie de _recetas_ para que, periódicamente, recoja en las _farmacias_ las "cajas" necesarias para completar el periodo de medicación previsto en cada _prescripción_ concreta.

**Nota:** Este es un procesamiento automático que se ejecutará en el momento de crear una _prescripción_ de un _medicamento_ dado para un determinado _paciente_ por parte de su _médico_ (ver historia HU-M4)
...
* Para cada _receta_ se mantendrá la siguiente información:
    ...
    - fecha de validez inicial, a partir de la cual el _paciente_ puede pasar por una _farmacia_ a recoger "cajas" del _medicamento_
    - fecha de validez final, pasada la cual no será posible recoger "cajas" del _medicamento_
    ...
    - _farmacia_ que sirvió la _receta_, inicialmente tendrá valor NULL (se establecerá la relación cuando se haga efectiva la entrega de las "cajas")
...
### Criterios de aceptación
* Se mostrará una lista con las _citas_ registradas para el _paciente_ logueado que tengan el estado `PLANIFICADA`, indicando sus datos esenciales (nombre del _médico_, fecha y hora, duración, estado). 
* Con el botón "Anular", se marcará el estado de la _cita_ seleccionada como `ANULADA`, actualizando la lista de _citas_ mostrada.
...
### Descripción
Como usuario _paciente_ logueado con contraseña, desde el formulario de "Visualización de mis recetas" se podrán visualizar la lista de _recetas_ pendientes de recoger y sus fechas de validez.
### Criterios de aceptación
* Se mostrará la lista de _recetas_ pendientes de ser servidas ordenada por fecha, de más próxima a más lejana.
* Para cada _receta_ se mostrará los datos del _medicamento_ de la correspondiente _prescripción_ (principio activo, fabricante), los datos del _médico_ que hizo la _prescripción_, las fechas de validez de cada _receta_ (inicial y final) y el número de unidades ("cajas") que se servirán en la _receta_ 
...
### Descripción
Como usuario _farmacia_, una vez logueado, desde la opción  "Gestión de receta" se accede a un formulario, donde introduciendo el nº de tarjeta sanitaria de un _paciente_ se accederá a la lista de _recetas_ que estén en vigor para ese _paciente_, visualizándose las _recetas_ existentes en el sistema disponibles para ser servidas al _paciente_ indicado desde la fecha actual.
### Criterios de aceptación
...
* Con el botón "Anotar _receta_ servida" se selecciona una de las recetas de la lista de _recetas_ disponibles para ser servidas (la fecha actual está dentro del periodo de validez de la _receta_) y se anota como servida (ver historia HU-F3)
